...
<details><summary>Explain why the <code>GET /new</code> action wouldn't be needed if your WordGame was called as a service in a true service-orientedarchitecture. </summary><p><blockquote>In a true SOA, the service that calls WordGame can generate an HTTP <CODE>POST</CODE> request directly.  The only reason for the <code>new</code> action is to provide the human Web user a way to generate that request.</blockquote></p></details>
<br />
